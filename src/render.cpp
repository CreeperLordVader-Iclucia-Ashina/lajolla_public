...
Image3 bidirectional_path_render(const Scene& scene) {
    int w = scene.camera.width, h = scene.camera.height;
    Image3 img(w, h);
    ...
    parallel_for([&](const Vector2i &tile) {
        ...
        for (int y = y0; y < y1; y++) {
            for (int x = x0; x < x1; x++) {
                ...
                int spp = scene.options.samples_per_pixel;
                for (int s = 0; s < spp; s++) {
                    radiance += bidirectional_path_tracing(scene, x, y, rng);
                }
                img(x, y) = radiance / Real(spp);
            }
        }
...
Image3 render(const Scene &scene) {
    ...
    } else if (scene.options.integrator == Integrator::BidirectionalPath) {
        return bidirectional_path_render(scene);
    }
    else {
        assert(false);
        return Image3();
