...
Spectrum eval_op::operator()(const DisneyDiffuse &bsdf) const {
    ...
    const Real FD_wi = 1.0 + (FD90 - 1.0) * pow(1.0 - abs(dot(vertex.geometry_normal, dir_in)), 5);
    const Real FD_wo = 1.0 + (FD90 - 1.0) * pow(1.0 - abs(dot(vertex.geometry_normal, dir_out)), 5);
