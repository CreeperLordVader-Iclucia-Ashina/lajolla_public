...
Spectrum eval_op::operator()(const DisneyDiffuse &bsdf) const {
    ...
    const Real FD_wi = 1.0 + (FD90 - 1.0) * pow(1.0 - NdotWin, 5);
    const Real FD_wo = 1.0 + (FD90 - 1.0) * pow(1.0 - NdotWout, 5);
    const Spectrum BaseColor = eval(bsdf.base_color, vertex.uv, vertex.uv_screen_size, texture_pool);
    const Spectrum f_baseDiffuse = BaseColor * M_1_PI * FD_wi * FD_wo * NdotWout;
    const Real FSS90 = Roughness * tmp * tmp;
    const Real FSS_wi = 1.0 + (FSS90 - 1.0) * pow(1 - NdotWin, 5);
    const Real FSS_wo = 1.0 + (FSS90 - 1.0) * pow(1 - NdotWout, 5);
